...
const STORAGE_PREFIX = "banners/advogado-mes/";
const FILE_TYPES = {
...
function validateFile(file, buffer) {
  ...
  if (!type) {
    const error = new Error("Formato não permitido. Use JPG, PNG, WebP ou GIF.");
    error.status = 415;
    throw error;
  }
...
export async function POST(request) {
  try {
    ...
    const storagePath = `${STORAGE_PREFIX}${year}/${month}/${crypto.randomUUID()}.${type.extension}`;
  ...
  try {
    ...
    if (Number(count || 0) > 0) {
      return json(
        {
          success: false,
          message: "A imagem está vinculada ao destaque e não pode ser removida.",
        },
        409,
      );
    }
    ...
    try {
      await registerBannerAudit(access.db, {
        ...
        reason: "Upload não utilizado do Advogado do Mês removido.",
        snapshot: { storagePath, context: "ADVOGADO_MES" },
      });
    } catch (auditError) {
      console.warn(
        "[Admin/AdvogadoMes/Upload] Arquivo removido sem auditoria disponível:",
        auditError.message,
      );
    }
