...
export async function POST(request) {
  try {
    ...
    const username = normalizeUsername(body?.usuario);
    ...
    if (
      ...
      return NextResponse.json(
        { success: false, message: "Informe usuário e senha válidos." },
        { status: 400 },
      );
    }
    ...
    const { data: advertiser, error } = await supabaseAdmin
      .from("anunciantes")
      .select(
        "id, username, nome_empresa, password, password_hash, ativo",
      )
      .ilike("username", username)
      .maybeSingle();

    if (error) {
      console.error("[Login anunciante] Erro no banco:", error);
      return NextResponse.json(
        { success: false, message: "Não foi possível realizar o acesso." },
        { status: 500 },
      );
    }
    ...
    if (advertiser.password_hash) {
      passwordValid = verifyAdvertiserPassword(
        password,
        advertiser.password_hash,
      );
    } else if (
      typeof advertiser.password === "string" &&
    ...
    const sessionToken = createAdvertiserSession(advertiser);
    const response = NextResponse.json({
      success: true,
      user: {
        id: advertiser.id,
        nome: advertiser.nome_empresa,
        usuario: advertiser.username,
        role: "ANUNCIANTE",
      },
    });

    response.cookies.set("sj_anunciante_session", sessionToken, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "lax",
      maxAge: advertiserSessionConfig.maxAge,
      path: "/",
    });

    return response;
  } catch (error) {
    console.error("[Login anunciante] Erro inesperado:", error);
    return NextResponse.json(
