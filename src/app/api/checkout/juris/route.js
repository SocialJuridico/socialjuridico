...
export async function POST(request) {
  ...
  try {
    ...
    const allowedPrices = allowedJurisPrices();
    if (!allowedPrices.size || !allowedPrices.has(priceId)) {
      return json(
        {
          success: false,
          message: "Preço não autorizado para compra de Juris.",
        },
        400,
      );
    }
    ...
    if (!price?.active || price.unit_amount === null) {
      return json(
        { success: false, message: "Preço de Juris indisponível." },
        400,
      );
    }
    ...
    if (internalCouponId) {
      reservation = await reserveCouponForCheckout(supabaseAdmin, {
        ...
        userId: user.id,
        expectedType: COUPON_TYPES.JURIS,
      });
      ...
      if (discountedAmount < 50) {
        const error = new Error(
          "O desconto deixa o valor abaixo da cobrança mínima de R$ 0,50. Use outro cupom ou pacote.",
        );
        error.status = 422;
        throw error;
      }
    ...
    if (sessionId) {
      try {
        ...
        console.warn(
          "[Checkout/Juris] Sessão não expirada após falha:",
          expireError.message,
        );
      }
    }
    ...
    console.error("[Checkout/Juris] Erro:", error);
    ...
    return json(
      {
        ...
        message:
          status < 500
            ? error.message
            : "Não foi possível iniciar a compra de Juris.",
      },
      status,
    );
  }
