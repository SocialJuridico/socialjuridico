...
export async function GET() {
  try {
    ...
    const { data: lawyers, error: lawError } = await supabaseAdmin
      .from("advogados")
      .select("id, email")
      .in("id", userIds);

    if (lawError) {
      console.warn("Aviso: Não foi possível buscar emails dos advogados:", lawError);
    ...
    const mergedData = requests.map(req => ({
      ...req,
      advogados: lawyers.find(l => l.id === req.user_id) || { email: "Não encontrado" }
    }));
