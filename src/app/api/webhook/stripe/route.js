...
async function handleCheckoutCompleted(session) {
  ...
  if (type === 'JURIS_PURCHASE') {
    ...
    else if (priceId === process.env.NEXT_PUBLIC_PRICE_JURIS_20) jurisAmount = 20;
    else if (priceId === process.env.NEXT_PUBLIC_PRICE_JURIS_50) jurisAmount = 50;
    ...
    await supabaseAdmin
      ...
      .update({ 
        is_premium: true,
        premium_until: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() // Exemplo 30 dias
      })
      .eq('id', userId);
  }
}
