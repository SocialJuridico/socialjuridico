...
export function useAdminLawyers() {
  ...
  const loadLawyers = useCallback(async () => {
    ...
    try {
      ...
      if (!lawyersResponse.ok || !lawyersData?.success) {
        throw new Error(lawyersData?.message || "Não foi possível carregar os advogados.");
      }
  ...
  const filteredLawyers = useMemo(() => {
    ...
    return lawyers.filter((lawyer) => {
      ...
      if (planFilter !== "ALL" && getPlanType(lawyer) !== planFilter) return false;
  ...
  const summary = useMemo(() => ({
    ...
    verified: lawyers.filter((item) => item.oab_verification_status === "VERIFIED").length,
    pending: lawyers.filter((item) => !item.oab_verification_status || item.oab_verification_status === "PENDING").length,
    errors: lawyers.filter((item) => item.oab_verification_status === "ERROR").length,
  ...
  const updateLocalLawyer = useCallback((lawyerId, action, value) => {
    setLawyers((current) => current.map((lawyer) => {
      ...
      if (action === "REMOVE_PRO") return { ...lawyer, is_premium: false, plan_type: "FREE", premium_expires_at: null };
      if (action === "ADD_JURIS") return { ...lawyer, balance: (lawyer.balance || 0) + Number(value || 0) };
      if (action === "REMOVE_JURIS") return { ...lawyer, balance: Math.max(0, (lawyer.balance || 0) - Number(value || 0)) };
      if (action === "SET_OAB_STATUS") return { ...lawyer, oab_verification_status: value };
      if (action === "UPDATE_OAB") return { ...lawyer, oab: value.oab, estado: value.estado };

      return lawyer;
    }));
  }, []);

  const performAction = useCallback(async (lawyer, action, value = null) => {
    setBusyId(lawyer.id);
  ...
  const confirmDelete = useCallback(async () => {
    ...
    setBusyId(lawyer.id);
    try {
      const response = await fetch(`/api/admin/advogados?id=${lawyer.id}`, { method: "DELETE" });
      const data = await response.json().catch(() => null);
      if (!response.ok || !data?.success) throw new Error(data?.message || "Falha ao excluir advogado.");
  ...
  const confirmReset = useCallback(async () => {
    ...
    setBusyId(lawyer.id);
    try {
      const response = await fetch(`/api/admin/advogados?id=${lawyer.id}`, { method: "PATCH" });
      const data = await response.json().catch(() => null);
      if (!response.ok || !data?.success) throw new Error(data?.message || "Falha ao resetar senha.");
      toast.success("Senha temporária definida com sucesso.");
      setModal(null);
    } catch (error) {
      console.error("[Admin/Advogados] Erro ao resetar senha:", error);
  ...
  const exportCsv = useCallback(() => {
    if (!filteredLawyers.length) return toast.error("Não há advogados para exportar.");
    exportLawyersCsv(filteredLawyers);
    toast.success("CSV de advogados exportado com sucesso.");
  }, [filteredLawyers]);

  const generatePdf = useCallback(async () => {
    if (!filteredLawyers.length) return toast.error("Não há advogados para o relatório.");
  ...
  const syncGoogle = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json().catch(() => null);
      if (!response.ok || !data?.success) {
        if (data?.message === "google_not_connected") setGoogleConnected(false);
  ...
  return {
    ...
    exportCsv,
    generatePdf,
    connectGoogle: () => { window.location.href = "/api/auth/google-contacts"; },
    syncGoogle,
  };
}
