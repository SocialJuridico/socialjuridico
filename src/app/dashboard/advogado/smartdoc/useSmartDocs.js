...
export function formatSmartDocSize(bytes) {
...
export function useSmartDocs() {
  ...
  const load = useCallback(
    async (targetPage = 1) => {
      ...
      const params = new URLSearchParams({
        ...
        type: filters.type,
        protection: filters.protection,
      });
      if (debouncedSearch) params.set("search", debouncedSearch);

      try {
        const response = await fetch(`/api/advogado/smartdoc?${params}`, {
          cache: "no-store",
        ...
        if (response.status === 401) {
          router.replace(
            `/login?redirectTo=${encodeURIComponent("/dashboard/advogado/smartdoc")}`,
          );
          return;
        }
        ...
        setPlan(data.plan || { type: "FREE", balance: 0, protectCost: 0 });
        setUsage(data.usage || EMPTY_USAGE);
        setMetrics(data.metrics || {});
        setPagination(data.pagination || {});
      } catch (loadError) {
        if (loadError.name === "AbortError") return;
        console.error("[SmartDoc] Falha ao carregar:", loadError);
  ...
  const uploadDocument = useCallback(
    async (event) => {
      ...
      if (protect && plan.type === "START" && plan.balance < plan.protectCost) {
        openJurisModal();
        toast.error(
      ...
      body.append("protect", String(protect));
      body.append("requestId", uploadRequestIdRef.current);

      try {
        const response = await fetch("/api/advogado/smartdoc", {
          method: "POST",
          body,
        });
        const data = await readJson(response);
  ...
  const deleteDocument = useCallback(
    async (document) => {
      ...
      const confirmed = window.confirm(
        `Excluir definitivamente o documento “${document.fileName}”?`,
      );
      if (!confirmed) return;
    ...
    }, [deletingId, items.length, load, pagination.page],
  );
