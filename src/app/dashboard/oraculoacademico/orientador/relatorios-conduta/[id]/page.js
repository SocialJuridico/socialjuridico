...
import {
  getOrientadorConductReport,
  markConductReportViewedByOrientador,
...
export default async function OrientadorRelatorioCondutaPage({ params }) {
  ...
  const report = await getOrientadorConductReport({ authUserId: context.authUserId, id });
  if (!report) notFound();

  await markConductReportViewedByOrientador({ authUserId: context.authUserId, id });

  return (
    <main className={styles.page}>
      <Link href="/dashboard/oraculoacademico/orientador/relatorios-conduta" className={styles.backLink}>
        Voltar
      </Link>
