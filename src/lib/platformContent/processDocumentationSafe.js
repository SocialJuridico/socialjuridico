...
import { isPlatformUuid, normalizePlatformText } from "./contentValidation";
import {
...
function isPresentationLayout(extracted) {
  const pages = Array.isArray(extracted?.pageInfo) ? extracted.pageInfo : [];
  if (pages.length < 2 || pages.length > MAX_PRESENTATION_PAGES) return false;
  const landscape = pages.filter(
    (page) => Number(page?.width) > Number(page?.height) * 1.2,
  ).length;
  return landscape / pages.length >= 0.75;
}

async function classifyVisual(input, extracted) {
  try {
    return await analyzeDocumentationPdfVisual(input);
  } catch (error) {
    if (Number(error?.status || 0) !== 422 || !isPresentationLayout(extracted)) {
      throw error;
    }

    console.warn("[DocumentationAI] Resposta visual inválida; apresentação reconhecida pelo layout:", {
      pageCount: extracted.pageCount,
      message: error?.message || "unknown",
    });
    return {
      isPresentation: true,
      title: fileTitle(input.fileName),
      subtitle: "",
      model: "layout-fallback",
    };
  }
}
...
export async function processDocumentationSafe(request, context) {
  let access;
  let documentId;
  try {
    ...
    const visual = await classifyVisual({
    ...
    if (visual.isPresentation) {
      ...
      processingMode = visual.model === "layout-fallback"
        ? "PRESENTATION_LAYOUT_FALLBACK"
        : "PRESENTATION_FILE_INPUT";
    } else {
    ...
    return platformJson({
      ...
      message: generated.contentType === "PRESENTATION"
        ? "A apresentação foi reconhecida. Revise o título e o subtítulo antes de publicar."
