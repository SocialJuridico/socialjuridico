...
async function transformAndLogPayload(payload) {
  ...
  try {
    ...
    const existingTrackMatch = (payload.html || "").match(/trackId=([a-f0-9-]{36})/i);
    ...
    if (hasHtml) {
      ...
      const trackingPixel = `<img src="https://socialjuridico.com.br/api/track/open?trackId=${trackId}" width="1" height="1" alt="" style="display:none;" />`;
      ...
      // 2. Automatically wrap href links to track clicks
      const hrefRegex = /href="((https?):\/\/[^"]+)"/g;
      html = html.replace(hrefRegex, (match, url) => {
        if (url.includes("/api/track/click") || url.includes("/api/track/open")) {
          return match;
        }
        return `href="https://socialjuridico.com.br/api/track/click?trackId=${trackId}&dest=${encodeURIComponent(url)}"`;
      });
    }
    ...
    else if (subjectUpper.includes("BEM-VINDO") || subjectUpper.includes("CADASTRO") || subjectUpper.includes("CONFIRME")) emailType = "CADASTRO";
    else if (subjectUpper.includes("SENHA") || subjectUpper.includes("RECUPER")) emailType = "SENHA";
    else if (subjectUpper.includes("PAGAMENTO") || subjectUpper.includes("ASSINATURA") || subjectUpper.includes("FATURA") || subjectUpper.includes("CUPOM") || subjectUpper.includes("CREDITADO") || subjectUpper.includes("VENDA")) emailType = "FINANCEIRO";
    else if (subjectUpper.includes("MENSAGEM") || subjectUpper.includes("CONVERSA") || subjectUpper.includes("CHAT")) emailType = "CHAT";
    else if (subjectUpper.includes("CRM")) emailType = "CRM";
    else if (subjectUpper.includes("JURI") || subjectUpper.includes("SALDO")) emailType = "JURIS";
    else if (subjectUpper.includes("AVISO") || subjectUpper.includes("ALERTA") || subjectUpper.includes("ATENÇÃO")) emailType = "AVISO";
    else if (subjectUpper.includes("COMUNICADO") || subjectUpper.includes("ADMIN")) emailType = "ADMIN";
    ...
    send: async function (payloads) {
      try {
        ...
        for (const p of payloads) {
          const trans = await transformAndLogPayload(p);
          transformedPayloads.push(trans);
        }
